...
class Reel_It_Upload_Handler {
    ...
    public static function handle_upload( $file ) {
        ...
        // Check file header for malicious patterns (read only 8 KB to avoid memory exhaustion on large videos).
        $fh = fopen( $file['tmp_name'], 'rb' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
        if ( ! $fh ) {
            return new WP_Error( 'invalid_content', __( 'Unable to read uploaded file.', 'reel-it' ) );
        }
        $header = fread( $fh, 8192 ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread
        fclose( $fh );
        if ( false !== strpos( $header, '<?php' ) ) {
