...
jQuery(document).ready(function ($) {
    ...
    // Why: escapeHtml is provided by reel-it-utils.js (loaded as a dependency).
    var escapeHtml = ReelItUtils.escapeHtml;
    ...
    function initTabs() {
        ...
        if ($('.reel-it-tab').length) {
            ...
            // BUG-12 fix: validate tab ID against known pane IDs to prevent DOM injection
            const validTabIds = $('.reel-it-tab-pane').map(function () { return this.id; }).get();
            if (lastTab && validTabIds.indexOf(lastTab) !== -1) {
                activateTab(lastTab);
            } else {
    ...
    /**
     * Build a feed-card HTML string from server response data.
     * Why: avoids a full-page reload after create/update by inserting DOM directly.
     */
    function buildFeedCardHtml(feed) {
        const thumbHtml = feed.thumbnail_url
            ? `<img src="${escapeHtml(feed.thumbnail_url)}" alt="${escapeHtml(feed.name)}" class="reel-it-feed-img">`
            : '<div class="reel-it-feed-placeholder"><span class="dashicons dashicons-video-alt3"></span></div>';
        const count = feed.video_count || 0;
        // translators: plural handled client-side for speed
        const countLabel = count === 1 ? count + ' Video' : count + ' Videos';
        const shortcode = `[reel_it feed_id="${escapeHtml(String(feed.id))}" use_feed="true"]`;

        return `
            <div class="reel-it-feed-item" data-feed-id="${escapeHtml(String(feed.id))}">
                <div class="reel-it-feed-content">
                    <div class="reel-it-feed-preview">${thumbHtml}</div>
                    <div class="reel-it-feed-info">
                        <div class="reel-it-feed-header">
                            <div class="reel-it-feed-name">${escapeHtml(feed.name)}</div>
                            <div class="reel-it-feed-actions">
                                <button type="button" class="button button-small button-primary reel-it-manage-videos" data-feed-id="${escapeHtml(String(feed.id))}" data-name="${escapeHtml(feed.name)}">${reelItSettings.strings.manageVideos}</button>
                                <button type="button" class="button button-small reel-it-edit-feed" data-feed-id="${escapeHtml(String(feed.id))}" data-name="${escapeHtml(feed.name)}" data-description="${escapeHtml(feed.description || '')}">${reelItSettings.strings.edit}</button>
                                <button type="button" class="button button-small button-secondary reel-it-delete-feed" data-feed-id="${escapeHtml(String(feed.id))}" data-name="${escapeHtml(feed.name)}">${reelItSettings.strings.delete}</button>
                            </div>
                        </div>
                        <div class="reel-it-feed-description">${escapeHtml(feed.description || '')}</div>
                        <div class="reel-it-feed-meta">
                            <span class="reel-it-video-count">${escapeHtml(countLabel)}</span>
                            <div class="reel-it-shortcode-wrapper">
                                <code class="reel-it-shortcode">${escapeHtml(shortcode)}</code>
                                <button type="button" class="button button-small reel-it-copy-shortcode" title="Copy Shortcode">
        ...
        $(document).on('click', '#reel-it-create-feed', function (e) {
            ...
            $.post(reelItSettings.ajaxUrl, data, function (response) {
                if (response.success) {
                    ...
                    // Why: DOM update instead of location.reload() — instant feedback
                    const feed = response.data.feed || { id: feedId || response.data.feed_id, name: name, description: desc, video_count: 0 };
                    if (isEdit) {
                        const $existing = $(`.reel-it-feed-item[data-feed-id="${feed.id}"]`);
                        if ($existing.length) {
                            $existing.replaceWith(buildFeedCardHtml(feed));
                        }
                    } else {
                        // Remove empty-state message if present
                        $('.reel-it-no-feeds').remove();
                        $('.reel-it-feeds-list').append(buildFeedCardHtml(feed));
                    }
                } else {
    ...
    function initVideoModal() {
        ...
        $(document).on('click', '#reel-it-add-videos, .reel-it-add-first-video', function (e) {
            ...
            frame.on('select', function () {
                ...
                $.when.apply($, promises).done(function () {
                    showNotification(reelItSettings.strings.videosAddedSuccess, 'success');
                    loadFeedVideos(currentFeedId);
                }).always(function () {
        ...
        $(document).on('click', '.reel-it-remove-video-btn', function (e) {
            ...
            $.post(reelItSettings.ajaxUrl, {
                ...
                if (response.success) {
                    ...
                    showNotification(reelItSettings.strings.videoRemoved, 'success');
                } else {
                    showNotification(reelItSettings.strings.videoRemoveFailed, 'error');
                    $item.css('opacity', '1');
                }
            });
        });
        ...
        function saveVideoOrder() {
            ...
            $.post(reelItSettings.ajaxUrl, {
                ...
                if (response.success) {
                    showNotification(reelItSettings.strings.videoOrderUpdated, 'success');
                } else {
                    showNotification(reelItSettings.strings.videoOrderFailed, 'error');
                }
            }).always(function () {
                $('.reel-it-video-toolbar .spinner').removeClass('is-active');
            });
        ...
        /**
         * Refresh a single feed card's thumbnail and video count from AJAX.
         * Why: avoids a full-page reload when closing the video manager modal.
         */
        function updateFeedCardMeta(feedId) {
            $.post(reelItSettings.ajaxUrl, {
                action: 'reel_it_get_feed_videos',
                nonce: reelItSettings.nonce,
                feed_id: feedId
                ...
                const $card = $(`.reel-it-feed-item[data-feed-id="${feedId}"]`);
                if (!$card.length) return;

                const videos = response.data.videos || [];
                const count = videos.length;
                const countLabel = count === 1 ? count + ' Video' : count + ' Videos';
                $card.find('.reel-it-video-count').text(countLabel);

                // Update thumbnail preview
                const $preview = $card.find('.reel-it-feed-preview');
                if (videos.length > 0 && videos[0].thumbnail) {
                    $preview.html(`<img src="${escapeHtml(videos[0].thumbnail)}" alt="" class="reel-it-feed-img">`);
                } else {
                    $preview.html('<div class="reel-it-feed-placeholder"><span class="dashicons dashicons-video-alt3"></span></div>');
                }
            });
        }
    }
    ...
    function initProductTagging() {
        ...
        $(document).on('click', '.reel-it-tag-product-btn', function (e) {
            ...
            if (!$notice.length) {
                $('#reel-it-product-search').before(
                    '<p class="reel-it-single-product-notice description" style="margin-bottom:8px;color:#9e6703;font-style:italic;">' +
                    (reelItSettings.strings.singleProductNotice || 'Only one product can be tagged per video.') +
                    '</p>'
                );
            }
        ...
        $('#reel-it-save-tags').on('click', function (e) {
            ...
            $.post(reelItSettings.ajaxUrl, {
                ...
                $btn.prop('disabled', false).text(reelItSettings.strings.saveTags);
            });
        });
        ...
        $(document).on('click', '.reel-it-copy-shortcode', function (e) {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(shortcode).then(() => {
                    showNotification(reelItSettings.strings.shortcodeCopied, 'success');
                    // Visual feedback
                    const originalIcon = $btn.html();
                    $btn.html('<span class="dashicons dashicons-yes"></span>');
                    setTimeout(() => $btn.html(originalIcon), 2000);
                }).catch(() => {
                    showNotification(reelItSettings.strings.shortcodeCopyFailed, 'error');
                });
            } else {
                // Fallback
