(function ($) {
    ...
    const ReelItAdmin = {
        ...
        handleFileUpload: function (file) {
            ...
            // BUG-09 fix: use server-configured max file size instead of hardcoded 50MB
            const maxSizeMB = (typeof reel_it_admin !== 'undefined' && reel_it_admin.maxFileSize) ? parseInt(reel_it_admin.maxFileSize, 10) : 50;
            const maxSize = maxSizeMB * 1024 * 1024;

            if (file.size > maxSize) {
                ReelItAdmin.showNotice('Video file is too large. Maximum size is ' + maxSizeMB + 'MB.', 'error');
                return;
            }
            ...
            xhr.addEventListener('load', function () {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ReelItAdmin.showNotice(reel_it_admin.strings.videoUploaded, 'success');
                            ReelItAdmin.addVideoToList(response.data);
                        } else {
                            ReelItAdmin.showNotice(response.data.message || reel_it_admin.strings.uploadFailed, 'error');
                        }
                    } catch (e) {
                        ReelItAdmin.showNotice(reel_it_admin.strings.uploadFailedInvalid, 'error');
                    }
                } else {
                    ReelItAdmin.showNotice(reel_it_admin.strings.uploadFailedServer, 'error');
                }
            });

            xhr.addEventListener('error', function () {
                ReelItAdmin.hideProgress();
                ReelItAdmin.showNotice(reel_it_admin.strings.uploadFailedNetwork, 'error');
            });
        ...
        // BUG-02 fix: escape all dynamic values to prevent XSS
        createVideoItem: function (videoData) {
            const thumbnailHtml = videoData.thumbnail ?
                `<img src="${escapeHtml(videoData.thumbnail)}" alt="${escapeHtml(videoData.title)}">` :
                '<div class="placeholder"><span class="dashicons dashicons-video-alt3"></span></div>';

            return $(`
                <div class="reel-it-video-item" data-video-id="${escapeHtml(videoData.id)}">
                    <div class="reel-it-video-thumbnail">
                        ${thumbnailHtml}
                    </div>
                    <div class="reel-it-video-info">
                        <div class="reel-it-video-title">${escapeHtml(videoData.title)}</div>
                        <div class="reel-it-video-meta">ID: ${escapeHtml(videoData.id)}</div>
                    </div>
        ...
        initBulkActions: function () {
            ...
            bulkActionButton.on('click', function (e) {
                ...
                if (!action) {
                    ReelItAdmin.showNotice(reel_it_admin.strings.selectAction, 'warning');
                    return;
                }

                if (selectedVideos.length === 0) {
                    ReelItAdmin.showNotice(reel_it_admin.strings.selectAtLeastOneVideo, 'warning');
                    return;
                }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        ReelItAdmin.showNotice(response.data.message || reel_it_admin.strings.actionFailed, 'error');
                    }
                },
                error: function () {
                    ReelItAdmin.showNotice(reel_it_admin.strings.actionFailedServer, 'error');
                }
            });
        },
        ...
        showProgress: function (percent) {
            const progressHtml = `
                <div class="reel-it-progress">
                    ...
                    <div class="reel-it-progress-text">${reel_it_admin.strings.uploading} ${Math.round(percent)}%</div>
                </div>
            `;
        ...
        updateProgress: function (percent) {
            ...
            $('.reel-it-progress-text').text(reel_it_admin.strings.uploading + ' ' + Math.round(percent) + '%');
        },
        ...
        // Show notice
        // BUG-03 fix: escape message to prevent XSS from server responses
        showNotice: function (message, type) {
            const notice = $(`
                <div class="reel-it-notice ${escapeHtml(type)}">
                    ${escapeHtml(message)}
                </div>
            `);
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.reel-it-remove-video', function () {
            if (confirm(reel_it_admin.strings.confirmRemoveVideo)) {
                $(this).closest('.reel-it-video-item').remove();
                ReelItAdmin.updateVideoData();
                ReelItAdmin.showNotice(reel_it_admin.strings.videoRemoved, 'success');
            }
        });
